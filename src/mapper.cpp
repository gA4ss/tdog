...
#define DEF_PAGE_SHIFT 12

Mapper::Mapper() {
	_page_shift = DEF_PAGE_SHIFT;
...
/* 设定页位移 */
void Mapper::set_page_shift(int shift) {
	if (shift == 0)
		_page_shift = DEF_PAGE_SHIFT;
	else
		_page_shift = shift;
...
unsigned Mapper::PAGE_START(unsigned x) {
	return x & ~_page_mask;
}

unsigned Mapper::PAGE_OFFSET(unsigned x) {
	return (x & _page_mask);
}

unsigned Mapper::PAGE_END(unsigned x) {
	return PAGE_START(x + _page_mask);
}

unsigned Mapper::PAGE_SIZE() {
	return _page_size;
}

unsigned Mapper::PAGE_MASK() {
	return _page_mask;
}
...
/**
  ...
  An ELF file's program header table contains one or more Elf32_Phdr::PT_LOAD
 ...
 */
int Mapper::phdr_table_load(int fd,
    ...
    unsigned page_min, page_max, page_offset;
    void* mmap_result;
...
void Mapper::phdr_table_unload(void* phdr_mmap, unsigned phdr_memsize) {
 ...
 */
unsigned Mapper::phdr_table_get_load_size(const Elf32_Phdr* phdr_table,
	...
	unsigned curr_vaddr = 0;
	unsigned curr_memsz = 0;
    ...
    for (unsigned short i = 0; i < phdr_count; ++i) {
		...
		curr_vaddr = get_te32(&phdr->p_vaddr);
        if (curr_vaddr < min_vaddr) {
            min_vaddr = curr_vaddr;
        }

		curr_memsz = get_te32(&phdr->p_memsz);
        if (curr_vaddr + curr_memsz > max_vaddr) {
            max_vaddr = curr_vaddr + curr_memsz;
        }
    }
 ...
 */
int Mapper::phdr_table_reserve_memory(const Elf32_Phdr* phdr_table,
									  unsigned short phdr_count,
									  void** load_start,
									  unsigned* load_size,
									  unsigned* load_bias) {
    ...
    unsigned size = phdr_table_get_load_size(phdr_table, phdr_count);
    if (size == 0) {
        // errno = EINVAL;
        return -1;
    }
    ...
    void* start = mmap(NULL, size, PROT_READ | PROT_WRITE, mmap_flags, -1, 0);
    if (start == MAP_FAILED) {
        return -1;
    }
    ...
    *load_size  = size;
    ...
    for (unsigned short i = 0; i < phdr_count; ++i) {
        ...
        if (type == PT_LOAD) {
            *load_bias = (unsigned)start - PAGE_START(vaddr);
            break;
        }
    }
    return 0;
 ...
 *   0 on success, -1 otherwise. Error code in errno.
 */
int Mapper::phdr_table_load_segments(const Elf32_Phdr* phdr_table,
									 unsigned short phdr_count,
									 unsigned load_bias,
									 int fd) {
	XASSERT(phdr_table);
    ...
    for (nn = 0; nn < phdr_count; nn++) {
        ...
        /* 加载内存 */
        unsigned seg_start = get_te32(&phdr->p_vaddr) + load_bias;
        unsigned seg_end   = seg_start + get_te32(&phdr->p_memsz);

        unsigned seg_page_start = PAGE_START(seg_start);
        unsigned seg_page_end   = PAGE_END(seg_end);

        unsigned seg_file_end   = seg_start + get_te32(&phdr->p_filesz);

        /* File offsets */
        unsigned file_start = get_te32(&phdr->p_offset);
        unsigned file_end   = file_start + get_te32(&phdr->p_filesz);

		/* 读取时,文件偏移也需要按照页对齐来读取 */
        unsigned file_page_start = PAGE_START(file_start);
        unsigned file_page_end   = PAGE_END(file_end);
		UNUSED(file_page_end);
        ...
        if ((get_te32(&phdr->p_flags) & PF_W) != 0 && 
			PAGE_OFFSET(seg_file_end) > 0) {
 ...
 */
int Mapper::phdr_table_set_load_prot(const Elf32_Phdr* phdr_table,
									 unsigned short    phdr_count,
									 unsigned          load_bias,
    ...
    for (; phdr < phdr_limit; phdr++) {
        if (get_te32(&phdr->p_type) != PT_LOAD || 
			(get_te32(&phdr->p_flags) & PF_W) != 0)
            continue;

        unsigned seg_page_start = PAGE_START(get_te32(&phdr->p_vaddr)) + 
			load_bias;
        unsigned seg_page_end   = PAGE_END(get_te32(&phdr->p_vaddr) + 
										   get_te32(&phdr->p_memsz)) + 
			load_bias;

        int ret = mprotect((void*)seg_page_start,
                           seg_page_end - seg_page_start,
						   PFLAGS_TO_PROT(get_te32(&phdr->p_flags)) | 
						   extra_prot_flags);
        if (ret < 0) {
            return -1;
        }
 ...
 */
int Mapper::phdr_table_protect_segments(const Elf32_Phdr* phdr_table,
										unsigned short phdr_count,
										unsigned load_bias) {
    return phdr_table_set_load_prot(phdr_table, phdr_count,
									load_bias, 0);
}
 ...
 *
 * 改变所有的在内存中的可加载段为可写.一旦完成重定位，则修改回元属性
 ...
 *   0 on error, -1 on failure (error code in errno).
 */
int Mapper::phdr_table_unprotect_segments(const Elf32_Phdr* phdr_table,
										  unsigned short    phdr_count,
										  unsigned          load_bias) {
    return phdr_table_set_load_prot(phdr_table, phdr_count,
 ...
 */
int Mapper::phdr_table_set_gnu_relro_prot(const Elf32_Phdr* phdr_table,
										  unsigned short    phdr_count,
										  unsigned          load_bias,
										  int               prot_flags) {
	XASSERT(phdr_table);
...
#define PT_GNU_RELRO	0x6474e552	/* Read-only after relocation */
    ...
    for (phdr = phdr_table; phdr < phdr_limit; phdr++) {
        if (get_te32(&phdr->p_type) != PT_GNU_RELRO)
            continue;
 ...
 * turn some of the pages of a writable Elf32_Phdr::PT_LOAD segment to read-only, as
 ...
 *   0 on error, -1 on failure (error code in errno).
 */
int Mapper::phdr_table_protect_gnu_relro(const Elf32_Phdr* phdr_table,
										 unsigned short    phdr_count,
										 unsigned          load_bias) {
    return phdr_table_set_gnu_relro_prot(phdr_table,
										 phdr_count,
										 load_bias,
										 PROT_READ);
}
...
/* 如果提供了.ARM.exidx节则返回它在内存中的地址与长度.
 *
 * 输入:
 *   phdr_table  -> 程序头表
 *   phdr_count  -> 程序头的数量
 ...
 */
int Mapper::phdr_table_get_arm_exidx(const Elf32_Phdr* phdr_table,
									 unsigned short phdr_count,
									 unsigned load_bias,
									 unsigned** arm_exidx,
	...
	XASSERT(phdr_table);
	XASSERT(arm_exidx);
	XASSERT(arm_exidx_count);

    const Elf32_Phdr* phdr = phdr_table;
    const Elf32_Phdr* phdr_limit = phdr + phdr_count;

    for (phdr = phdr_table; phdr < phdr_limit; phdr++) {
        if (get_te32(&phdr->p_type) != PT_ARM_EXIDX)
            continue;

        *arm_exidx = (unsigned*)(load_bias + get_te32(&phdr->p_vaddr));
        *arm_exidx_count = (unsigned)(get_te32(&phdr->p_memsz) / 8);
        return 0;
    }
    *arm_exidx = NULL;
    *arm_exidx_count = 0;
 ...
 */
const Elf32_Phdr* Mapper::phdr_table_get_loaded_phdr(const Elf32_Phdr*   phdr_table,
													 unsigned short      phdr_count,
													 unsigned            load_bias) {
	XASSERT(phdr_table);
    ...
    for (phdr = phdr_table; phdr < phdr_limit; phdr++) {
        if (get_te32(&phdr->p_type) == PT_PHDR) {
            loaded = load_bias + get_te32(&phdr->p_vaddr);
            goto CHECK;
        }
    }
    ...
    for (phdr = phdr_table; phdr < phdr_limit; phdr++) {
        if (get_te32(&phdr->p_type) == PT_LOAD) {
            if (get_te32(&phdr->p_offset) == 0) {
                unsigned  elf_addr = load_bias + get_te32(&phdr->p_vaddr);
                const Elf32_Ehdr* ehdr = (const Elf32_Ehdr*)(void*)elf_addr;
                unsigned  offset = get_te32(&ehdr->e_phoff);
                loaded = (unsigned)ehdr + offset;
                goto CHECK;
            }
            break;
        }
...
CHECK:
    ...
    for (phdr = phdr_table; phdr < phdr_limit; phdr++) {
        if (get_te32(&phdr->p_type) != PT_LOAD)
            continue;
        unsigned seg_start = get_te32(&phdr->p_vaddr) + load_bias;
        unsigned seg_end   = get_te32(&phdr->p_filesz) + seg_start;
...
void Mapper::free_phdr_ptr(struct phdr_ptr** ptr) {
	XASSERT(ptr);
	XASSERT(*ptr);
	if ((*ptr)->phdr_mmap)
		phdr_table_unload((*ptr)->phdr_mmap, (*ptr)->phdr_size);
	delete *ptr;
	*ptr = NULL;
}
